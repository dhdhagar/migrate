...
for target in "${targets[@]}"; do
  ...
  accelerate launch train_grpo.py --date $(date "+%Y-%m-%d_%H-%M-%S") --model "ekinakyurek/marc-8B-finetuned-llama3" --strategy "online" --task "arc" --num_train_epochs 1 --batch_size 10 --num_generations 10 --use_permutations --learning_rate 3e-4 --online_max_completion_length 256 --target "$target" --nll_weight=1e-2 --grpo_weight=1 --wandb_prefix "nll_grpo_online" --wandb_tags "nll,grpo,online" --beta=0.01
done
