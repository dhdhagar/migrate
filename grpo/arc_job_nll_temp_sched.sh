...
for target in "${targets[@]}"; do
  ...
  accelerate launch train_grpo.py --date $(date "+%Y-%m-%d_%H-%M-%S") --model "ekinakyurek/marc-8B-finetuned-llama3" --strategy "Greedy_Single" --task "arc" --num_train_epochs 1 --batch_size 10 --num_generations 10 --all_combinations --learning_rate 3e-4 --online_max_completion_length 256 --target "$target" --nll_weight=1 --grpo_weight=0 --online_temperature 0.5 --wandb_prefix "sft_temp_sched" --use_train_temp_schedule
done
