...
for target in "${targets[@]}"; do
  # Migrate (OPRO)
  accelerate launch train_grpo_arc.py --task "arc" --date "$current_time" --model "$model_name" --target "$target" --trl_seed 2 --strategy "migrate_opro" --num_train_epochs 1 --wandb_prefix "arc_small_migrate" --wandb_tags "grpo,arc,migrate,top1" --min_training_size 64 --max_training_size 64 --inf_batch_size 16 --batch_size 16 --num_generations 16 --online_max_completion_length 1024 --inf_num_samples 0 --learning_rate 1e-5 --lora_alpha 32 --lora_rank 128 --num_iterations 1 --migrate_sampling_strategy "opro" --greedy_topk 1 --migrate_gamma 4 --migrate_alpha 11 --migrate_beta 1 --use_induction
done
